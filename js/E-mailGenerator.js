...
fetchApiKey();
// Fetch the AI-generated template based on the selected E-mail type
async function fetchTemplate(E-mailType, E-mailTone, additionalInfo) {
if (!OPENROUTER_API_KEY) {
        alert("API key is missing. Please try again.");
        return;
    }
    const prompt = `Generate a professional template for a ${E-mailType} with a tone that is ${E-mailTone}. Include the following additional information: "${additionalInfo}". Use placeholders in [ ] for missing details.`;
...
function loadInputFields(placeholders) {
    ...
    generateButton.innerText = "Generate Final E-mail";
    generateButton.onclick = () => replacePlaceholders(placeholders);
    container.appendChild(generateButton);
}

// Replace placeholders with user-provided inputs and refine the E-mail
async function replacePlaceholders(placeholders) {
    const template = document.getElementById("template").value;
    const finalE-mailArea = document.getElementById("finalE-mail");
let finalE-mail = template;
finalE-mailArea.value = "Generating Final E-mail...";
finalE-mailArea.disabled = true; // Disable editing while loading


    placeholders.forEach((placeholder) => {
        const userInput = document.getElementById(placeholder).value.trim();
        if (userInput) {
            finalE-mail = finalE-mail.replace(new RegExp(`\\[${placeholder}\\]`, "g"), userInput);
        } else {
    ...
    try {
        ...
        console.error("Error refining E-mail:", error);
        finalE-mailArea.value = "An error occurred while generating the E-mail.";
    } finally {
        finalE-mailArea.disabled = false; // Re-enable editing after completion
    }
}
...
async function refineEmail(E-mail, E-mailTone) {
 ...
 // Replace with your valid API key
        ...
        console.error("Error refining E-mail:", error);
        alert("Failed to refine the E-mail. Showing the original E-mail instead.");
        return E-mail; // Return the original E-mail if refinement fails
    }
}
...
const loadingIndicator = document.getElementById("loadingIndicator");
   ...
   if (!E-mailType || !E-mailTone) {
    alert("Please select both a E-mail type and a tone.");
    return;
}
    ...
    try {
        ...
        const template = await fetchTemplate(E-mailType, E-mailTone, additionalInfo);
        ...
        console.error("Error handling E-mail generation:", error);
    } finally {
...
// Copy Final E-mail to Clipboard
document.getElementById("copyFinalButton").addEventListener("click", () => {
    const finalText = document.getElementById("finalE-mail").value;
    navigator.clipboard.writeText(finalText)
        .then(() => alert("Final E-mail copied to clipboard!"))
        .catch(err => console.error("Failed to copy text:", err));
});

// Download Final E-mail as PDF
document.getElementById("downloadFinalButton").addEventListener("click", () => {
    const { jsPDF } = window.jspdf; // Access jsPDF
    const finalText = document.getElementById("finalE-mail").value;

    if (finalText.trim() === "") {
        alert("Final E-mail is empty. Cannot generate PDF.");
        return;
    }
